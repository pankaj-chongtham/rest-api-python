...
def create_app(app_name):
    ...
    with open(routes_file_path, 'w') as routes_file:
        routes_file.write(
            ...
            f"  # we are running in a bundle\n"
            f"  CURRENT_PATH = os.path.dirname(sys.executable)\n"
            f"else:\n"
            f"  # we are running in a normal Python environment\n"
            f"  CURRENT_PATH = os.path.dirname(os.path.abspath(__file__))\n"
            f"CURRENT_PATH = os.path.normpath(os.path.join(CURRENT_PATH, '..', '..', '..'))\n\n"
            f"# CONFIG FILE\n"
            f"config_filename = os.path.join(CURRENT_PATH, 'config.ini')\n"
            f"config_obj = configparser.ConfigParser()\n"
            f"config_obj.read(config_filename)\n\n"
            f"jwt_enabled = config_obj.getboolean('API_SETTING', 'jwtenabled')\n\n"
            f"# Wrapper function that conditionally applies jwt_required()\n"
            f"def conditional_jwt_required(func):\n"
            f"  if jwt_enabled:\n"
            f"      return jwt_required()(func)\n"
            f"  else:\n"
            f"      return func\n\n"
            f"@{app_name}_bp.route('/ping')\n"
            f"@conditional_jwt_required\n"
            f"def ping():\n"
